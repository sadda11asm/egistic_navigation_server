...
from app.worker.egistic_navigation.base_geometry.geom_utils import min_dist
...
from app.worker.egistic_navigation.base_geometry.poly_utils import ThePoly, TheChunk
...
class FieldPoly(ThePoly):
    ...
    def get_extension_lines(self, parent=None, show=False):
        ...
        for i_chunk, the_chunk in enumerate(chunks):
            ...
            for i_point, xy in enumerate(the_chunk.xy[:-1]):
                ...
                if the_chunk.touches_parent and (
                        not convex_chunk.touches(xy) or xy_point in the_chunk.baseline): continue
                ...
                for chunk_line in the_chunk[xy]['lines']:
                    ...
                    # line[0].plot('Start')
                    # line[1].plot('End')
                    extension_line = chunk_line.extend_from(xy,
                                                            the_main_poly.cover_line_length)  # .plot(text='extension_line')
                    # plt.show()
    ...
    def get_min_altitude(self, use_mp=False):
        if use_mp:
            alt_getter = lambda _exterior_line: (
                FieldPoly(
                    shaff.rotate(self.geometry, 180 - _exterior_line.angle, _exterior_line[0].geometry)).get_altitude(),
                _exterior_line)
            altitude_data = gsup.ParallelTasker(alt_getter).set_run_params(_exterior_line=self.all_lines).run(
                sleep_time=1e-4)
            ...
            for exterior_line in self.all_lines:
                cost = FieldPoly(
                    shaff.rotate(self.geometry, 180 - exterior_line.angle, exterior_line[0].geometry)).get_altitude()
    ...
    def get_field_lines_old(self, offset_distance, base_line, show=False):
        ...
        # 		field_lines_data[sub_field.id,k].extend(lines)
            ...
            for i_parent_line, _parent_line in enumerate(shrinked_field.all_lines):
                if base_line is None or abs(
                        (_parent_line.angle + 180) % 180 - (base_line.angle + 180) % 180) < min_dist:
    ...
    def get_subfields(self, start_node, offset_distance, show=False):
        ...
        for path in get_paths(self, self.adjacency_info, start_node):
            ...
            shrinked_resultant_poly = remaining_field.geometry.buffer(-offset_distance / 2,
    ...
    def get_min_cost(self, linewidth):
        ...
        for exterior_line in self.all_lines:
            ...
            # self.plot()
            rotated_poly = FieldPoly(shaff.rotate(self.geometry, 270 - 1e-6 - exterior_line.angle,
                                                  exterior_line[0].geometry))  # .plot('rotated_poly')
            # exterior_line.plot()
    ...
    def get_visible_poly(self, observer_point, as_field=True):
        ...
        if as_field:
            return FieldPoly.from_(visible_poly)
        else:
            return visible_poly
    ...
    def get_subparcel_centers(self, parcel_area, random_state=123, show='', grid_factor=0.1):
        ...
        if n_clusters < 2 :
            return "the given area is large to generate clusters"
    ...
    if use_mp:
        if not len(polygons[1:]) == 0:
            chunks_of_optimum_sub_polygons = mi.flatten(
                gsup.ParallelTasker(perform_optimization).set_run_params(field_poly=polygons[1:]).run(
                    len(polygons[1:])))
...
def main():
    ...
    # n=int(field_poly.polygon.area/(25*1e3))
    # logger.debug('n: %s',n)
    ...
    for i in range(100):
        field_poly = FieldPoly.synthesize(cities_count=100, hole_count=2)
